...
#sources: old tic tac toe code and google
...
allp = [allrows, allcolumns, alldiag]
allc = [callrows, callcolumns, calldiag]
...
def player():
    ...
    while True:  #puting the input in a loop and it will only break if you put in a number on the board
        copy = 0  #break the while true loop if the user put correct input
    ...
    move = move-1
    new_list[move] = "o"
    for ways in allp: #columns, rows, diags
        for each in ways: #looking at num
            if(move+1) in each:
                each.remove(move+1)
